////////////////////////////////////////
//   Today : Tue May 10 12:25:04 2022 //
//  Author : rasauq1122               //
// Problem : BOJ2214                  //
////////////////////////////////////////
...
signed main() {
    ...
    while (true) {
        ...
        rep(i, 0, r+1) {
            colsum[i][0] = 0;
            rep(j, 0, c) colsum[i][j+1] = colsum[i][j] + (garo[i][j] != '*');
        }

        int rowsum[21][21];
        rep(i, 0, c+1) {
            rowsum[i][0] = 0;
            rep(j, 0, r) rowsum[i][j+1] = rowsum[i][j] + (sero[j][i] != '*');
        }

        int ans = 0;
        rep(i, 0, min(r, c)) {
            rep(x, 0, r-i) rep(y, 0, c-i) {
                int len = i+1;
                int csum1 = colsum[x][y+len] - colsum[x][y];
                int csum2 = colsum[x+len][y+len] - colsum[x+len][y];
                int rsum1 = rowsum[y][x+len] - rowsum[y][x];
                int rsum2 = rowsum[y+len][x+len] - rowsum[y+len][x];

                if (csum1 == csum2 && csum2 == rsum1 && rsum1 == rsum2 && rsum2 == len) ans++;
            }
        }
