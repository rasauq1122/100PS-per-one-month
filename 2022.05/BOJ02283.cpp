...
signed main() {
    ...
    // seg[i] = the count of segment within [i, i+1]
    repe(i, 0, MAX-2) seg[i+1] += seg[i]; 

    // seg[i] = partial sum of segments within [0, i+1]
    repe(i, 0, MAX-2) seg[i+1] += seg[i];

    for (int s = 0, e = 1 ; e <= MAX ; e++) {
        while (s+1 < e && seg[e] - seg[s] > k) s++;
        if (seg[e] - seg[s] == k) return cout << s << ' ' << e << endl, 0;
    }
