...
signed main() {
    ...
    priority_queue <tiii, vt, decltype(&cmp)> pq(cmp);

    vi t(n), p(n), q(n);
    rep(i, 0, n) cin >> t[i] >> p[i] >> q[i];

    pq.push({p[0]-t[0], q[0], 1});
    
    int T = t[0];
    for (int i = 1 ; i < n ;) {
        for ( ; i < n && t[i] <= T ; i++) pq.push({p[i]-t[i], q[i], i+1});
        
        auto [tmp, d, idx] = pq.top();
        pq.pop();

        cout << idx << ' ';
        if (i < n) T = max(T + d, t[i]);
    }

    for (; !pq.empty() ; pq.pop()) cout << get<2>(pq.top()) << ' ';
    cout << endl;
}