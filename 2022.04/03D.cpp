...
int dfs_sum(int idx) {
	if (idx >= h) return v[idx];

	sum[idx][0] = dfs_sum(2*idx);
	sum[idx][1] = dfs_sum(2*idx+1);
	return v[idx] + sum[idx][0] + sum[idx][1];
}

int dfs_path(int idx) {
	if (idx >= h) return v[idx];
	path[idx][0] = dfs_path(2*idx);
	path[idx][1] = dfs_path(2*idx+1);
	return v[idx] + max(path[idx][0], path[idx][1]);
}
...
void dfs(int idx, int nx, int ny, int gx, int gy, bool capture) {
	if (ny < gx || gy < nx) {
		other_sum += sum[idx][0] + sum[idx][1] + v[idx];
		if (capture) candi.pb(idx);
		return;
	}
	if (gx <= nx && ny <= gy) return;

	if (!capture) other_sum += v[idx];
	int len = (ny - nx)/2;
	dfs(2*idx, nx, nx + len, gx, gy, capture || 2*idx == lca);
	dfs(2*idx+1, ny - len, ny, gx, gy, capture || 2*idx+1 == lca);
}
...
signed main() {
	...
	for (int i = 1 ; i + k - 1 <= h ; i++) {
		...
		dfs(1, 1, h, i, i+k-1, lca == 1);

		vi candi_path(2);
		each(candi, now) candi_path.pb(v[now] + max(path[now][0], path[now][1]));
		...
		while (lca/2) {
			bool dir = (lca/2)*2 == lca;
			lca /= 2;
			par_path = max(par_path, base + v[lca] + path[lca][dir]);
			base += v[lca];
		}

		sort(all(candi_path), greater<int>());
		
		nax = max(nax, tree_sum - other_sum + max(candi_path[0] + candi_path[1], candi_path[0] + par_path));
	}
