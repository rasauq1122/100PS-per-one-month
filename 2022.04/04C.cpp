...
int foo(int idx, int bit, bool open) {
	int& ret = dp[idx][bit][open];

	if (ret != -1) return ret;
	if (idx == 50000) return ret = 1;

	ret = 0;

	if (!open && BIT(bit, 1) && BIT(bit, 2)) rep(i, 1, 4) ret += foo(idx+1, i<<1, false);
	else {
		ret += foo(idx+1, 3<<1, true);
		rep(i, 1, 4) ret += foo(idx+1, i<<1|1, false);
	}
...
signed main() {
	...
	repe(i, 1, 50000) psum.pb(psum.back() + foo(50001-i, 0, 0));

	TC() {
		rd1(n);
		pf2(foo(50001-n, 0, 0), (psum[n-1]*2 + foo(50001-n, 0, 0))*n%10007);
	}
}