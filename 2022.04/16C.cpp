...
int foo(int x, int y, bool mode) {
    int &ret = dp[x][y][mode];
    if (vis[x][y][mode]) return ret;
    if (x == n-1 && y == m-1 && mode) return ret = arr[x][y];

    ret = -LINF;
    if (mode) {
        if (x < n-1) ret = max(ret, arr[x][y] + foo(x+1, y, 1));
    }
    else {
        ret = max(ret, arr[x][y] + foo(x, y, 1));
        if (x) ret = max(ret, arr[x][y] + foo(x-1, y, 0));
    }

    if (y < m-1) ret = max(ret, arr[x][y] + foo(x, y+1, mode));
