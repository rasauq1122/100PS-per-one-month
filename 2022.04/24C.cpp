////////////////////////////////////////
//   Today : Sun Apr 24 13:37:01 2022 //
//  Author : rasauq1122               //
// Problem : BOJ2218                  //
////////////////////////////////////////
...
int foo(int i, int j) {
    ...
    if (i == a && j == b) return ret = 0;
    if (i == a) return vis[i][j] = 2, ret = foo(i, j+1);
    if (j == b) return vis[i][j] = 1, ret = foo(i+1, j);

    pii nax = {-MOD, 0};
    nax = max({nax, mp(foo(i+1, j), 1), mp(foo(i, j+1), 2), mp(foo(i+1, j+1) + score[A[i]][B[j]], 3)});
    tie(ret, vis[i][j]) = nax;
...
signed main() {
    ...
    cin >> n >> a >> b;
    score = vec<int>(n+1, n+1);
    repe(i, 1, n) repe(j, 1, n) cin >> score[i][j];
