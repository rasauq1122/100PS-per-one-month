...
signed main() {
    ...
    TC() {
        double p;
        char color, coupon, payment;
        cin >> p >> color >> coupon >> payment;
        
        cout << '$';
        if (payment == 'P') cout << p*(100-sale[color])*(100-sale[coupon])/10000 << endl;
        else {
            int q = p*(100-sale[color])*(100-sale[coupon]);
            
            if (q%1000 <= 500) q -= q%1000;
            else q += (1000-q%1000);
