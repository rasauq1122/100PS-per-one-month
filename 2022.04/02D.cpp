...
int foo(int i, int j) {
	...
	ret = max({0LL, foo(i+1, j) - 2, foo(i, j+1) - 2, foo(i+1, j+1) + (a[i] == b[j] ? 3 : -2)});

	return ret;
}

pii btk(int i, int j) {
	if (i == n || j == m) return {i, j};

	int ret = max({0LL, foo(i+1, j) - 2, foo(i, j+1) - 2, foo(i+1, j+1) + (a[i] == b[j] ? 3 : -2)});

	if (ret == 0) return {i, j};
	if (ret == foo(i+1, j) - 2) return btk(i+1, j);
	if (ret == foo(i, j+1) - 2) return btk(i, j+1);
	return btk(i+1, j+1);
}
...
signed main() {
	...
	rep(i, 0, n) rep(j, 0, m) nax = max(nax, {foo(i, j), {i, j}});
	pii en = btk(nax.ss.ff, nax.ss.ss);

	pf1(nax.ff);
	rep(i, nax.ss.ff, en.ff) cout << a[i];
	pf0();
	rep(i, nax.ss.ss, en.ss) cout << b[i];
	pf0();
}