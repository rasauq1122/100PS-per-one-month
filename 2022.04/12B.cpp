...
signed main() {
	...
	int q = n/3;
	// 3*n = a + 2*b 를 만족하는 음이 아닌 정수쌍 (a, b) : a[n]
	...
	// 3*n + 1 = a + 2*b 를 만족하는 음이 아닌 정수쌍 (a, b) : b[n]
	...
	// 3*n + 2 = a + 2*b 를 만족하는 음이 아닌 정수쌍 (a, b) : c[n]
	
	// c[0] = 2, c[1] = 3, c[2] = 5, c[3] = 6
	
	// c[i+2] = c[i] + 3 (i > 0)
	// c[0] = 2, c[1] = 3;

	int e = q/2+1, o = (q+1)/2;
	pf1((e*(e+1)/2*3 - (2 - (n%3==2))*e + o*(o+1)/2*3 - o*(n%3==0))%1000000);
}