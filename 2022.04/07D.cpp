...
int getR(int i, int j) {
	if (i > j) return 0;
	return lsum[j+1] - lsum[i] - i*(psum[j+1] - psum[i]);
}

int getL(int i, int j) {
	if (i > j) return 0;
	return rsum[i] - rsum[j+1] - (n-j-1)*(psum[j+1] - psum[i]);
}

signed main() {
	FASTIO();

	repe(i, 2, 30000) if (!npri[i]) {
		pri.pb(i);
		for (int j = 2*i ; j <= 30000 ; j += i) npri[j] = 1;
	}

	len = sz(pri);
	twin = vi(len);
	rep(i, 1, len) if (pri[i-1] + 2 == pri[i]) twin[i] = 1;
    ...
    while (true) {
		...
		rep(i, 0, n) psum[i+1] = psum[i] + v[i];

		rep(i, 0, n) lsum[i+1] = lsum[i] + v[i]*(i+1);
		
		pere(i, 1, n) rsum[i-1] = rsum[i] + v[i-1]*(n-i+1);

		int nin = getL(0, n-3) + getR(n-1, n-1);

		if (n-1 >= 7) {
			rep(i, 0, n-7) {
				int sum = v[i+1] + v[i+3] + v[i+4] + v[i+6];
				int L = getL(0, max(i-1, -1LL)); 
				int R = getR(min(i+8, n), n-1);
				nin = min(nin, sum+L+R);
			}
		}
		...
		rep(k, 0, len) {
			...
			rep(i, 0, n-p) {
				int mid = (i + i+p)/2;
				int sumL = getR(i+1, mid);
				int sumR = getL(mid+1, i+p-1);
				int L = getL(0, max(i-1, -1LL));
				int R = getR(min(i+p+1, n), n-1);

				if (twin[k]) {
					int tmp;
					mid = (i + i+p-2)/2;
					sumL = getR(i+1, mid);
					sumR = getL(mid+1, i+p-3);

					tmp = sumL + sumR + v[i+p-1];

					mid = (i+2 + i+p)/2;
					sumL = getR(i+3, mid);
					sumR = getL(mid+1, i+p-1);

					tmp = min(tmp, sumL + sumR + v[i+1]);
					nin = min(nin, L + R + tmp);
					continue;
				}

				nin = min(nin, L + sumL + sumR + R);
			}
		}
