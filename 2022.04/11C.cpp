...
double S(Point a, Point b, Point c) {
	double A = a.x*b.y + b.x*c.y + c.x*a.y;
	double B = a.y*b.x + b.y*c.x + c.y*a.x;
...
class Line {
	...
	bool check(Line other) {
	    int ab = ccw(p, q, other.p)*ccw(p, q, other.q);
		int cd = ccw(other.p, other.q, p)*ccw(other.p, other.q, q);

		if (ab <= 0 && cd <= 0) return other.p <= q && p <= other.q;
	...
	Point solve(Line other) {
		double b1 = p.x - q.x, b2 = other.p.x - other.q.x;
		double a1 = q.y - p.y, a2 = other.q.y - other.p.y;

		double c1 = a1*p.x + b1*p.y;
		double c2 = a2*other.p.x + b2*other.p.y;

		double con = 1.0/(b2*a1 - b1*a2);
		double mat[2][2] = {{b2, -b1}, {-a2, a1}};

		return Point(con*(mat[0][0]*c1 + mat[0][1]*c2), con*(mat[1][0]*c1 + mat[1][1]*c2));
	}
};

double bx, cx, cy, ex, ey;
...
signed main() {
	...
	TC() {
		...
		FIX(3);
		cout << S(a, b, A)-0.0005 << ' ' << S(b, c, B) -0.0005 << ' ' << S(c, d, C) -0.0005 << ' ' << S(d, a, D) -0.0005 << ' ' << S(A, B, C) + S(A, C, D) -0.0005<< ' ';
		// cout << S(a, b, A)/160 << ' ' << S(b, c, B)/160 << ' ' << S(c, d, C)/160 << ' ' << S(d, a, D)/160 << ' ' << (S(A, B, C) + S(A, C, D))/160 << ' ';

		FIX(0);
		pf1(ceil(A.distant(B) + B.distant(C) + C.distant(D) + D.distant(A)));
		// pf1(floor((A.distant(B) + B.distant(C) + C.distant(D) + D.distant(A))*16.5)+0.5);
	}
}