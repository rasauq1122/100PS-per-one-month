...
signed main() {
    ...
    rep(i, 0, n) {
        int tmp, rst;
        string user;
        cin >> tmp >> user >> rst >> tmp >> tmp >> tmp >> tmp;
        if (rst == 4) nap[user] = {nap[user].ff, 1};
        else if (!nap[user].ss) nap[user] = {nap[user].ff + 1, 0};
    }

    int ac = 0, wa = 0;
    for (auto it = nap.begin() ; it != nap.end() ; it++) {
        if (it->first == "megalusion") continue;
        if (!(it->second).ss) continue;
        ac++;
        wa += (it->second).ff;
    }

    FIX(9);
    if (ac+wa == 0) cout << 0 << endl;
    else cout << 100.0*ac/(ac+wa) << endl;
}