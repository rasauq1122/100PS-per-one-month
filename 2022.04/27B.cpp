////////////////////////////////////////
//   Today : Wed Apr 27 09:18:07 2022 //
//  Author : rasauq1122               //
// Problem : BOJ24465                 //
////////////////////////////////////////
...
int stella(int m, int d) {
    int cut[12] = {19, 18, 20, 19, 20, 21, 22, 22, 22, 22, 22, 21};
    rep(i, 0, 12) if (m == i+1) return (11+i + (d > cut[i]))%12;
    exit(-1);
}
...
signed main() {
    ...
    rep(i, 0, n) {
        int M, D;
        cin >> M >> D;
        if (!exist[stella(M, D)]) candi.pb({M,D});
    }

    int len = sz(candi);
    if (!len) return cout << "ALL FAILED", 0;
