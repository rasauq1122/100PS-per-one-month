...
vi vis;
int dfs_sum(int r, int i) {
	vis[i] = true;

	sum[r][i]++;
	each(edge[i], e) {
		int nxt = e.ff;
		if (!vis[nxt]) sum[r][i] += dfs_sum(r, nxt);
	}

	return sum[r][i];
}

void dfs(int r, int i, bool init) {
	vis[i] = true;
	
	each(edge[i], e) {
		int nxt = e.ff, sel = e.ss;
		if (!vis[nxt]) {
			if (init) prob[sel] = 1.0*sum[r][nxt]/(sum[r][r]-1);
			else prob[sel] *= 1.0*sum[r][nxt]/(sum[r][r]-1);
			dfs(r, nxt, init);
		}
	}
}
