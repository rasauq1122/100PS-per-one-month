...
signed main() {
	...
	TC() {
		...
		rep(i, 0, 3) {
			int d;
			rd1(d);
			D[i] = {d, {min(i,(i+1)%3), max(i,(i+1)%3)}};
		}
		sort(D, D+3);

		int a, b, c;
		if (D[0].ss.ff == D[1].ss.ff) b = 0, a = D[0].ss.ss, c = D[1].ss.ss;
		else if (D[0].ss.ss == D[1].ss.ss) b = 2, a = D[0].ss.ff, c = D[1].ss.ff;
		else b = 1, a = (D[0].ss.ff ? 2 : 0), c = (D[0].ss.ff ? 0 : 2);

		impif(D[0].ff + D[1].ff < D[2].ff);
		impif((D[0].ff + D[1].ff - D[2].ff)%2);

		int dt = (D[0].ff + D[1].ff - D[2].ff)/2;

		x[a] = 1, y[a] = 1;
		x[c] = 1 + min(D[2].ff - 1, 800000000LL - 1), y[c] = 2 + max(D[2].ff - 800000000, 0LL);
