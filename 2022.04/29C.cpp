////////////////////////////////////////
//   Today : Fri Apr 29 23:37:44 2022 //
//  Author : rasauq1122               //
// Problem : BOJ22253                 //
////////////////////////////////////////
...
int foo(int idx, int val, int par) {
    int &ret = dp[idx][val];
    ...
    // choose this one (if you can)
    // ... and pass to child:
    if (arr[idx] >= val) {
        ret++; // terminate path here

        each(edge[idx], nxt) {
            if (nxt == par) continue;
            ret += foo(nxt, arr[idx], idx);
            ret %= MOD;
        }
    }
