...
int n;
vs v;
string s[4][10] = {{"", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"},
...
signed main() {
	...
	repe(i, 1, 9) s[3][i] = s[0][i] + "hundred";
	...
	repe(i, 1, 999) {
		string now = "";
		if (i/100) now += s[3][i/100];
		if (i%100 <= 9) now += s[0][i%100];
		else if (i%100 <= 19) now += s[1][i%100-10];
		else now += s[2][i/10 - i/100*10] + s[0][i%10];
