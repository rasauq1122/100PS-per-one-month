...
void btk() {
	if (sz(ans) == n-1) {
		...
		each(ans, idx) {
			piii e = edge[idx];
			int u = e.ff, v = e.ss.ff, c = e.ss.ss;
		...
		repe(i, 1, n) if (degree[i] > b[i]) return;
		nin = min(nin, {sum, ans});

		return;
	}

	rep(i, sz(ans) ? ans.back()+1 : 0, m) {
		ans.pb(i);
		btk();
		ans.bp();
	}
...
signed main() {
	...
	par = vi(n+1);
	btk();
	if (nin.ff == INF) pf1("NO");
	else {
		pf1("YES");
		rep(i, 0, n-1) pf2(edge[nin.ss[i]].ff, edge[nin.ss[i]].ss.ff);
	}
}