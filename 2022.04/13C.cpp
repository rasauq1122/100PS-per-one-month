...
void btk() {
    if (sz(ban) == k+1) {
        ...
        while (!q.empty()) {
            ...
            bool chk = false;
            each(ban, b) if (b == a) chk = true;

            int cost = chk ? 0 : work[a];
            cum[a] += cost;

            each(edge[a], nxt) {
                cum[nxt] = max(cum[a], cum[nxt]);
                if (!--now[nxt]) q.push(nxt); 
            }
        }
        int nax = 0;
        each(cum, p) nax = max(p, nax);
        nin = min(nax, nin);
