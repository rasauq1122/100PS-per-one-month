...
signed main() {
	...
	rep(i, 0, t) {
		...
		if (c == 'C') {
			rd1(arg2);
			if (!(item[p][arg1] && item[p][arg2])) rog.pb(k);
			if (item[p][arg1]) item[p][arg1]--;
			if (item[p][arg2]) item[p][arg2]--;
		}
		else if (c == 'M') place[p] = arg1;
		else if (c == 'F') {
			if (arg1 != place[p]) rog.pb(k);
			item[p][arg1]++;
		}
		else {
			if (place[p] != place[arg1]) {
				rog.pb(k);
				ban[p] = 1;
			}
		}
	...
	vi banp;
	repe(i, 1, n) if (ban[i]) banp.pb(i);
	pf1(sz(banp));
	if (sz(banp)) pvec(banp);
}