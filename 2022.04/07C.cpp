...
signed main() {
	...
	pqueue <piii, vpp, greater<piii>> pq;
	dist = wi(n+1, vi(k+1, INF*MOD));

	pq.push({0, {a, k}});
	while (!pq.empty()) {
		int cum = pq.top().ff;
		int idx = pq.top().ss.ff;
		int rem = pq.top().ss.ss;
		pq.pop();

		if (dist[idx][rem] != INF*MOD) continue;
		dist[idx][rem] = cum;
		...
		each(edge[idx], e) {
			...
			repe(j, 0, rem) {
				bool okay = dist[nxt][rem-j] == INF*MOD;
				if (!okay) continue;
				repe(i, rem+1-j, k) {
					if (dist[nxt][i] <= cum + T[k-rem+j][t]) {
						okay = false;
						break;
					}
				}

				if (okay) pq.push({cum + T[k-rem+j][t], {nxt, rem-j}});
			}
	...
	int nin = INF*MOD;
	repe(i, 0, k) nin = min(nin, dist[b][i]);
