...
int bfs() {
	dist = vector<wi>(r, wi(c, vi(n+1, INF)));
	dist[0][0][0] = 0;
	...
	int ndx[50][50][4], ndy[50][50][4];
	rep(i, 0, r) rep(j, 0, c) rep(k, 0, 4) {
		int ni = i + dx[k];
		int nj = j + dy[k];
		while (safe(ni, nj, r, c) && arr[i][j] == arr[ni][nj]) {
			ni += dx[k];
			nj += dy[k];
		}
		ndx[i][j][k] = ni;
	...
	while (!q.empty()) {
		...
		q.pop();
		if (z == n) return dist[x][y][z];
		
		rep(i, 0, 4) {
			int nx = ndx[x][y][i];
			int ny = ndy[x][y][i];

			if (!safe(nx, ny, r, c)) continue;
			if (dist[x][y][z] + 1 >= dist[nx][ny][z]) continue;
			dist[nx][ny][z] = dist[x][y][z] + 1;
			q.push({nx, ny, z});
		}
		
		if (z < n && s[z] == arr[x][y]) {
			if (dist[x][y][z] + 1 >= dist[x][y][z+1]) continue;
