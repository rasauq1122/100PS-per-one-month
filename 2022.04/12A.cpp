...
signed main() {
    ...
    TC() {
		...
		int sum = fi + ff*12 + mi + mf*12;
		if (c == 'B') sum += 5;
		else sum -= 5;

		string nin = to_string((sum/2-4+sum%2)/12)+'\''+to_string((sum/2-4+sum%2)%12)+'\"';
		string nax = to_string((sum/2+4)/12)+'\''+to_string((sum/2+4)%12)+'\"';
		pf4("Case #"+to_string(TEST_NUM)+":", nin, "to", nax);
	}
}