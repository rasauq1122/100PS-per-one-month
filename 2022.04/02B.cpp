...
int hp_me, atk_me, hp_ene, atk_ene;
int p, s;

signed main() {
	FASTIO();
	
	rd4(hp_me, atk_me, hp_ene, atk_ene);
	rd2(p, s);

	int t = (hp_me-1)/atk_ene + 1;
	int hp = hp_ene;
	
	int unit1 = (hp-p-1)/atk_me + 1;
	t -= unit1;
	hp -= unit1*atk_me;

	if (hp <= 0 && t >= 0) return pf1("Victory!"), 0;

	hp += s;
	int unit2 = (hp-1)/atk_me + 1;
	t -= unit2;

	if (t >= 0) return pf1("Victory!"), 0; 
