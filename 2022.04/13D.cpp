...
void dijk(int idx) {
    ...
    while (!pq.empty()) {
        ...
        dist[now] = cum;
        if (mth[now] != -1) pdist[idx][mth[now]] = cum;

        each(edge[now], e) {
            auto [nxt, cost] = e;
            if (dist[nxt] == MAX) pq.push({cost+cum, nxt});
        }
    }
}
...
int foo(int idx, int bit) {
    int &ret = dp[idx][bit];

    if (ret != -1) return ret;
    if (__builtin_popcount(bit) == p) return ret = pdist[idx][p+1];

    ret = MAX;
    rep(i, 0, p) {
        if (BIT(bit, i)) continue;
        ret = min(ret, pdist[idx][i] + foo(i, bit|(1<<i)));
    }
